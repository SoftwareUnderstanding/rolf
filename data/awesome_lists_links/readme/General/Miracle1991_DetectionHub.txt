...
We provide a simple webcam demo that illustrates how you can use `DetectionHub` for inference:
```bash
cd demo
...
For the following examples to work, you need to first install `DetectionHub`.
...
For that, all you need to do is to modify `DetectionHub/config/paths_catalog.py` to
...
```bash
python /path_to_DetectionHub/tools/train_net.py --config-file "/path/to/config/file.yaml"
```
...
```bash
export NGPUS=8
python -m torch.distributed.launch --nproc_per_node=$NGPUS /path_to_DetectionHub/tools/train_net.py --config-file "path/to/config/file.yaml" MODEL.RPN.FPN_POST_NMS_TOP_N_TRAIN images_per_gpu x 1000
```
...
```bash
export NGPUS=8
python -m torch.distributed.launch --nproc_per_node=$NGPUS /path_to_DetectionHub/tools/test_net.py --config-file "configs/e2e_mask_rcnn_R_50_FPN_1x.yaml" TEST.IMS_PER_BATCH 16
```
To calculate mAP for each class, you can simply modify a few lines in [coco_eval.py](https://github.com/facebookresearch/maskrcnn-benchmark/blob/master/DetectionHub/data/datasets/evaluation/coco/coco_eval.py). See [#524](https://github.com/facebookresearch/maskrcnn-benchmark/issues/524#issuecomment-475118810) for more details.
...
For a full example of how the `COCODataset` is implemented, check [`DetectionHub/data/datasets/coco.py`](DetectionHub/data/datasets/coco.py).

Once you have created your dataset, it needs to be added in a couple of places:
- [`DetectionHub/data/datasets/__init__.py`](DetectionHub/data/datasets/__init__.py): add it to `__all__`
- [`DetectionHub/config/paths_catalog.py`](DetectionHub/config/paths_catalog.py): `DatasetCatalog.DATASETS` and corresponding `if` clause in `DatasetCatalog.get()`
...
To enable your dataset for testing, add a corresponding if statement in [`DetectionHub/data/datasets/evaluation/__init__.py`](DetectionHub/data/datasets/evaluation/__init__.py):
```python
