...
阅读R-CNN系列论文笔记注释版，这里主要在CPU上运行，若要在GPU上运行修改回即可，做了如下改变满足CPU运行测试：
```
1. config.py中修改
__C.USE_GPU_NMS = False

2. lib/fast_rcnn/nms_wrapper.py中注释
...
4. tools/test_net.py中修改
caffe.set_mode_cpu()

5. tools/train_net.py中修改
caffe.set_mode_cpu()

6. caffe-fast-rcnn/CMakeLists.txt中修改
caffe_option(CPU_ONLY  "Build Caffe without CUDA support" ON) # TODO: rename to USE_CUDA

7. caffe-fast-rcnn/src/caffe/layer_factory.cpp中注释
