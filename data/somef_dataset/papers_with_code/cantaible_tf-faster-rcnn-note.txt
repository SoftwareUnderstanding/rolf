# 给自己看的
我的环境是ubuntu cpu tensorflow 与[代码来源](https://github.com/endernewton/tf-faster-rcnn)的GPU环境不同，需要做一些修改。
参考博客[Faster R-CNN tensorflow版本，cpu下运行demo](https://blog.csdn.net/sinat_33486980/article/details/81045315)
- /lib/model/config.py:USE_GPU_NMS=False
- 修改lib/model/nms_wrapper.py
- lib/setup.py
...
**Note**:
  - Due to the randomness(隨意) in GPU training with Tensorflow especially for VOC, the best numbers are reported (with 2-3 attempts) here. According to my experience, for COCO you can almost always get a very close number (within ~0.2%) despite the randomness.
