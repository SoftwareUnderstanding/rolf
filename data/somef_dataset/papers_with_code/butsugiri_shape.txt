...
- A basic usage of this repository should be almost the same as the original [OpenNMT-py](https://github.com/OpenNMT/OpenNMT-py) framework; please follow the documentations available there.
- The only difference is that `--pe_noise_size` is available in the training config.
- This value corresponds to the maximum shift for SHAPE (i.e., `K` in the paper)
- Setting --pe_noise_size`` to `0` is equivalent to the vanilla Transformer
- You may find `config_shape` useful: this directory contains all the configuration `*.yaml` files used in our experiments.

## Acknowledgements

This repository is based on [OpenNMT-py](https://github.com/OpenNMT/OpenNMT-py), a wonderful and state-of-the-art NMT framework.

I would like thank the contributors of the original codebase.
