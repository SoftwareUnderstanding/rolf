...
parser.add_argument('--links_csv', help='Path to csv with collected links')
parser.add_argument('--readme_folder', help='Path to the folder with readme files')
...
elif args.links_csv and args.readme_folder:
	reader = csv.DictReader(open(args.links_csv), delimiter=';')
	data = {key: [] for key in ['Label', 'Repo', 'Text']}
	for row in reader:
		filename = '_'.join(row['Repo'].split('/')[-2:]) + '.txt'
		print(filename)
		with open(args.readme_folder + filename) as f:
